...
type Backend struct {
	...
	AccessPointsByBSSID map[string]gonetworkmanager.AccessPoint
}
...
func New() (wifi.Backend, error) {
	...
	return &Backend{
		...
		AccessPointsByBSSID: make(map[string]gonetworkmanager.AccessPoint),
	}, nil
}
...
func (b *Backend) BuildNetworkList(shouldScan bool) ([]wifi.Connection, error) {
	...
	newAccessPointsByBSSID := make(map[string]gonetworkmanager.AccessPoint)
	...
	for ssid, aps := range ssidsAccessPoints {
		...
		for _, subAP := range aps {
			...
			newAccessPointsByBSSID[bssid] = subAP
	...
	b.AccessPointsByBSSID = newAccessPointsByBSSID
...
func (b *Backend) ActivateConnection(ssid, bssid string) error {
	...
	if bssid != "" {
		...
		ap, apOK = b.AccessPointsByBSSID[bssid]
		if !apOK {
