...
func New(logger *slog.Logger) (wifi.Backend, error) {
	logger = logger.With("backend", "darwin")
...
func (b *Backend) BuildNetworkList(shouldScan bool) ([]wifi.Connection, error) {
	// Get current network
	b.logger.Debug("getting current network")
	...
	if err == nil {
		...
		b.logger.Warn("failed to get current network", "error", err)
	}

	b.logger.Debug("listing preferred networks", "current", currentSSID)
	// Get known networks
	...
	b.logger.Debug("scanning for networks")
...
func (b *Backend) ActivateConnection(ssid string) error {
	...
	if err != nil {
		...
		b.logger.Debug("no secret found for network", "ssid", ssid, "error", err)
		password = ""
	}
...
func (b *Backend) JoinNetwork(ssid string, password string, security wifi.SecurityType, isHidden bool) error {
	b.logger.Info("joining network", "ssid", ssid, "hidden", isHidden, "security", security)
	cmd := exec.Command("networksetup", "-setairportnetwork", b.WifiInterface, ssid, password)
	...
	b.logger.Debug("adding to preferred networks", "ssid", ssid, "security", securityType)
